...
const CustomForm = ({ status, message, onValidated }) => {
  ...
  return (
    <div className="w-full md:w-1/2 flex flex-col justify-center font-roboto py-24">
      <h1
        className="text-6xl font-staat text-ketic text-center pb-10"
        data-aos="fade-up"
        data-aos-delay="50"
      >
        Stay up to date.
      ...
      <div className="w-full flex justify-center">
        <div
          className="flex justify-between rounded items-center p-2 border-2 bg-gray w-3/4"
          data-aos="fade-up"
        >
          <input
          ...
          />
          <button
            className="bg-ketic rounded text-gray px-5 h-14"
            onClick={submit}
          >
            {status === "sending" ? (
              <span>Sending...</span>
      ...
      {status === "error" && (
        <div
          className="text-red text-center pt-4"
          dangerouslySetInnerHTML={{ __html: message }}
        />
      )}
      {status === "success" && (
        <div
          className="text-ketic text-center pt-2"
          dangerouslySetInnerHTML={{ __html: message }}
        />
      )}
    </div>
